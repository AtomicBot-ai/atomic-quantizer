...
| `driver/vast.py`, `driver/remote.py` | rent/probe/destroy (port of atomic-forge rent_race.sh), scp + tmux + follow |
| `tests/` | replay of the published releases, refusal cases |
...
The token goes to a box as `/root/.hf_env` (0600) and nowhere else: not into a
...
log that moved last, so a long imatrix run is not mistaken for a hang.
...
- `AtomicChat/<Stem>-GGUF-metrics` (dataset): `bf16/`, `inventory.json`,
  `imatrix/`, `kld/base-neutral.kld[.NN.part]` + manifest, `ladder/`,
  `logs/quantize-*`, `logs/kld-neutral--*`, `logs/verify-*`, `results/rows/*.json`,
  `results.json`.
- `AtomicChat/<Stem>-abliterated` (private first), `AtomicChat/<Stem>-NVFP4`.

## Not done yet

- model card from `results.json` (atomic-forge `src/make_card.py` knows only UD-);
